...
int main(int, char**)
{
	...
	unsigned bufferSize = smallerSize.width * smallerSize.height * sizeof(char);
	...
	clock_gettime( CLOCK_REALTIME, &bigstart);
        // Mat filterframe = Mat(inputFrame.size(), CV_8UC3);
        // Mat edge_x,edge_y,edge,edge_inv;
    	// cvtColor(inputFrame, grayframe, CV_BGR2GRAY);


		// copyMakeBorder(grayframe, grayframe, blurIterations, blurIterations, blurIterations, blurIterations, BORDER_REPLICATE);

		uchar* input = (uchar*)clEnqueueMapBuffer(queue, input_buf, CL_TRUE, CL_MAP_WRITE, 0, bufferSize, 0, NULL, &write_event[0], &errcode);
	...
	clock_gettime( CLOCK_REALTIME, &start);
		...
		checkError(status, "Failed to set input argument");
		status = clSetKernelArg(kernel, 1, sizeof(cl_mem), &input_buf);
		checkError(status, "Failed to set output argument");
		...
		float* output  = (float*)clEnqueueMapBuffer(queue, output_buf
		, CL_TRUE, CL_MAP_READ, 0, bufferSize, 0, NULL, NULL, &errcode);
		checkError(errcode, "Failed to map output");

		Mat outputFrame = Mat(smallerSize.height, smallerSize.width, CV_8UC3, output);
		...
		tot += (double)( end.tv_sec - start.tv_sec ) + (double)( end.tv_nsec - start.tv_nsec ) / BILLION;
